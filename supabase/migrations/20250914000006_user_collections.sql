...
RETURNS TRIGGER AS $$
BEGIN
  IF NEW.slug IS NOT NULL THEN NEW.slug := LOWER(NEW.slug); END IF;
  NEW.updated_at := NOW();
  RETURN NEW;
...
-- Replace manage_collection to use user_collections table
...
BEGIN
  ...
  END IF;
  IF p_user_id IS NULL THEN RAISE EXCEPTION 'User must be authenticated'; END IF;
  ...
  ELSE
    -- delete: remove memberships then the collection
...
-- Replace get_user_collections to pull from user_collections and left join memberships for counts
...
BEGIN
  ...
  ORDER BY CASE WHEN b.collection_slug = ANY(reserved_slugs) THEN 0 ELSE 1 END, b.collection_label;
END; $$ LANGUAGE plpgsql SECURITY DEFINER;
