-- Move typeSubCategories logic to a new migration to avoid modifying existing pushed migration

-- 1. Text Match with Filters (Updated with typeSubCategories)
...
BEGIN
  ...
  WHERE p.text_vector IS NOT NULL
    ...
    AND (
      (filters->'typeSubCategories' IS NULL OR jsonb_array_length(filters->'typeSubCategories') = 0)
      OR (p.type_category = ANY(ARRAY(SELECT jsonb_array_elements_text(filters->'typeSubCategories'))))
    )
    -- UPDATED: Regex match for Fit (Exact word in CSV)
    AND (
...
-- 2. Image Match with Filters (Updated with typeSubCategories)
