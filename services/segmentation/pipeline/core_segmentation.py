...
def _load_schp_model(
    ...
    if checkpoint_path is None:
        ...
        checkpoint_path = os.path.join(parent_dir, "checkpoints", "exp-schp-201908261155-lip.pth")
    ...
    schp_root = os.environ.get("SCHP_ROOT")
    if not schp_root:
        parent_dir = os.path.dirname(current_dir)
        schp_root = os.path.join(parent_dir, "Utils", "Self-Correction-Human-Parsing")
        if not os.path.exists(schp_root):
            schp_root = os.path.join(current_dir, "Utils", "Self-Correction-Human-Parsing")
