...
let job: { job_id: string; current_state: string; error_count: number };
const failed: Array<{ jobId: string; message: string }> = [];
const retried: Array<{ jobId: string; message: string }> = [];

mock.module('../domain/job-catalog', () => ({
  getJob: async () => job,
  markJobFailed: async (jobId: string, message: string) => { failed.push({ jobId, message }); },
  markJobRetrying: async (jobId: string, message: string) => { retried.push({ jobId, message }); },
  updateState: async () => {},
  updateJob: async () => {},
  listJobs: async () => [],
  insertJob: async () => job,
...
beforeEach(() => { failed.length = 0; retried.length = 0; });
...
describe('dispatch', () => {
  ...
  for (const state of ['awaiting_hitl_identification', 'awaiting_hitl_segmentation']) {
    test(`a stray dispatch on '${state}' does not fail the job`, async () => {
      job = { job_id: 'job-1', current_state: state, error_count: 0 };
  ...
  test('every HITL state is covered by the guard, not by a handler', async () => {
    for (const state of HITL_STATES) {
      job = { job_id: 'job-x', current_state: state, error_count: 0 };
  ...
  test('terminal states are still skipped silently', async () => {
    for (const state of TERMINAL_STATES) {
      job = { job_id: 'job-t', current_state: state, error_count: 0 };
  ...
  test('a genuinely unknown state IS still failed', async () => {
    job = { job_id: 'job-2', current_state: 'not_a_real_state', error_count: 0 };
