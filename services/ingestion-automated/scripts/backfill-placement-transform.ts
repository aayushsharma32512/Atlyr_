#!/usr/bin/env bun
/**
 * Backfill the editor-space `transform` onto placement artifacts that don't have one.
 *
 ...
 *
 * What it does, per job whose LATEST placement artifact lacks a transform:
...
import { catalogId, placementEntryFrom, writePlacementEntry } from '../src/domain/catalog';
...
async function runModal(row: Row): Promise<{ transform: Transform; mannequin: string | null }> {
  const url =
...
async function persist(row: Row, transform: Transform, mannequin: string | null): Promise<void> {
  const existingMannequin = (row.data?.selectedMannequin as string | undefined) ?? null;
  const patch: Record<string, unknown> = { transform, transformBackfilledAt: new Date().toISOString() };
  ...
  const entry = placementEntryFrom(
    { transform, warp: [], selectedMannequin: existingMannequin ?? mannequin },
    row.product_gender_type,
  );
  if (entry) await writePlacementEntry(catalogId(row.job_id), entry.key, entry.entry);
}
...
async function main(): Promise<void> {
  ...
  console.log(`placement artifacts missing a transform: ${all.length}`);
  ...
  const worker = async (): Promise<void> => {
    for (let row = queue.shift(); row; row = queue.shift()) {
      ...
      try {
        const { transform, mannequin } = await runModal(row);
        await persist(row, transform, mannequin);
        ok++;
